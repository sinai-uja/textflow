...
class LiwcAnalyzer(Analyzer):
    """
    A class that provides methods to obtain Liwc features from the text of a sequence.

    Attributes:
        LiwcFile: a pandas DataFrame with the Liwc lexicon.
        tokenizer: a nltk tokenizer that split the texts in words.
    """

    def __init__(self, pathFile=os.path.join(os.path.dirname(__file__), 'liwc_norm.csv'), tokenizer = RegexpTokenizer("[\w']+")):
        """
        Create a Liwc analyzer.

        Args:
            pathFile: the path of the Liwc lexicon.
            tokenizer: a nltk tokenizer that split the texts in words.
        """
        self.liwcFile = pd.read_csv(pathFile).drop(['Unnamed: 0'],axis=1)
    ...
    def search_terms(self, arrayText):
        ...
        Returns:
            A list with the results of the analysis of the correspondant text with Liwc lexicon.  The result of the analysis of each text is a list composed of 4 sublist. The 4 sublist are maximum values, the minimum values, the standard deviation values and the mean values found for each Liwc feature (anger, fear, sadness, positive, negative).
        """
        arrayResults =[]
        for text in arrayText:
            ...
            for i in tkn_phrase:
                if i in lst_terms:
                    elm = self.liwcFile.loc[self.liwcFile['Términos'] == i].values.flatten().tolist()
                    elms.append(elm)
            dir_f = {}
            for i in elms:
                dir_f[str(i[0])] = [self.tofloat(x) for x in [i[1], i[2], i[3], i[4], i[5]]]
            ar_res = []
            try:
