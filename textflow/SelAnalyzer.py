...
class SelAnalyzer(Analyzer):
    """
    A class that provides methods to obtain Sel features from the text of a sequence.

    Attributes:
        selFile: a pandas DataFrame with the Sel lexicon.
        tokenizer: a nltk tokenizer that split the texts in words.
    """

    def __init__(self, pathFile=os.path.join(os.path.dirname(__file__), 'sel_norm.csv'), tokenizer = RegexpTokenizer("[\w']+")):
        """
        Create a Sel analyzer.

        Args:
            pathFile: the path of the Sel lexicon.
            tokenizer: a nltk tokenizer that split the texts in words.
        """
    ...
    def search_terms(self, arrayText):
        """
        Function that analyzes a list of texts to extract the Sel metrics.

        Args:
            arrayText: list that contains the texts that we want to analyze
        Returns:
            A list with the results of the analysis of the correspondant text with Sel lexicon.  The result of the analysis of each text is a list composed of 4 sublist. The 4 sublist are maximum values, the minimum values, the standard deviation values and the mean values found for each Sel feature (anger, fear, joy, sadness).
        """
        arrayResults =[]
        for text in arrayText:
            tkn_phrase = self.tokenizer.tokenize(text)
            lst_terms = self.selFile['Términos'].tolist()
            elms = []
            for i in tkn_phrase:
                if i in lst_terms:
                    elm = self.selFile.loc[self.selFile['Términos'] == i].values.flatten().tolist()
                    elms.append(elm)
            dir_f = {}
            for i in elms:
